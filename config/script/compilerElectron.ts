...
export function startCompilerElectron(port = 80) {
  ...
  const rollupOptions1 = getRollupOptions1();
  function watchFunc() {
    // 一旦到达这里，所有资源都可用
    // once here, all resources are available
    const watcher = rollup.watch(rollupOptions);
    const watcher1 = rollup.watch(rollupOptions1);
  ...
  if (argv.watch) {
    ...
    spinner.start();
    rollup
      .rollup(rollupOptions1)
      .then((build) => {
        spinner.stop();
        console.log(TAG, chalk.green('Electron build successed.'));
        build.write(rollupOptions1.output as OutputOptions);
      })
      .catch((error) => {
        spinner.stop();
        console.log(`\n${TAG} ${chalk.red('构建报错')}\n`, error, '\n');
    ...
    rollup
      ...
      .then((build) => {
        spinner.stop();
        console.log(TAG, chalk.green('Electron preload successed.'));
