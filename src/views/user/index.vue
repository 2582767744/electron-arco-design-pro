    ...
    <div class="container" :style="{ display: 'flex', width: '100%', height: '100%' }">
        <a-card :style="{ width: '100%', height: '100%' }" title="用户列表">
            ...
            <a-table :data="renderList" stripe :pagination="false" :bordered="false" page-position="bottom"
                :style="{ height: '100%' }" :draggable="{}" @change="((data: TableData[], currentData: TableData[]) =>handleChange(data,currentData))">
                <template #columns>
...
const handleChange = (data: TableData[], currentData: TableData[]) => {
    console.log(data);
...
<script lang="ts">
export default {
    name: 'UserList1', // If you want the include property of keep-alive to take effect, you must name the component
};
</script>
